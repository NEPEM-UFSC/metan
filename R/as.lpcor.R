...
as.lpcor <- function(...) {
  ...
  if (length(which(sapply(data, function(x) identical(nrow(x),
    ...
    cli::cli_abort("All matrices in the list must be a square matrix. Please, check and fix.")
  }
  if (length(unique(unique(sapply(data, function(x) dim(x)))[1,
                                                             1:length(data)])) != 1) {
    cli::cli_abort("All matrices in the list must be the same dimension. Please, check and fix.")
  }
