...
#' *j*th block; *m* is the grand mean (fixed); \eqn{g_i} is the effect
#' of the *i*th genotype; \eqn{r_j} is the effect of the *j*th
...
#'  where where \eqn{y_{ijk}}
#' is the response variable of the *i*th genotype in the *k*th block
#' of the *j*th replicate; *m* is the intercept, \eqn{t_i} is the
#' effect for the *i*th genotype \eqn{r_j} is the effect of the *j*th
...
gafem <- function(.data,
  ...
  if (verbose == TRUE) {
    ...
    switch(design,
           rcbd =  cli::cli_h2("One-way ANOVA table (Randomized complete block design)"),
           alpha = cli::cli_h2("One-way ANOVA table (Alpha-lattice design)"))
    ...
    if (length(which(unlist(lapply(listres, function(x) {
      ...
      cli::cli_inform("{names(which(unlist(lapply(listres, function(x) { as.numeric(x[['anova']][2, 6]) })) > prob))}")
      cli::cli_text("")
    }
  }
