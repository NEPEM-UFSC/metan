...
#' \deqn{WAASB_i = \sum_{k = 1}^{p} |IPCA_{ik} \times EP_k|/ \sum_{k =
#' 1}^{p}EP_k}
#'
#' where \eqn{WAASB_i} is the weighted average of absolute scores of the
#' *i*th genotype; \eqn{IPCA_{ik}} is the score of the *i*th genotype in the
#' *k*th Interaction Principal Component Axis (IPCA); and \eqn{EP_k} is the
...
waasb <- function(.data,
  ...
  for (var in 1:nvar) {
    ...
    ovmean <- mean(data$Y)
    if (minimo < 2) {
      cli::cli_alert_warning("The analysis is not possible.")
      cli::cli_alert_warning("The number of environments and number of genotypes must be greater than 2")
    }
    if(ind_anova == TRUE){
      if(missing(block)){
  ...
  if (verbose == TRUE) {
    ...
    if (length(which(unlist(lapply(listres, function(x) {
      ...
      if(any(chek_na_waasb == TRUE)){
        cli::cli_h2("The following traits had p-value for GE interaction = 1")
        cli::cli_inform("{names(which(chek_na_waasb == TRUE))}")
        cli::cli_inform("WAASBY value for these traits is based on mean performance only (PctResp)")
      }
    } else {
...
print.waasb <- function(x, export = FALSE, blup = FALSE, file.name = NULL, digits = 4, ...) {
  if (!inherits(x, "waasb")) {
    cli::cli_abort("The object must be of class 'waasb'")
  }
  if (export == TRUE) {
  ...
  for (i in 1:length(x)) {
    ...
    cli::cli_h2("Individual fixed-model analysis of variance")
    print(var$individual$individual)
    cli::cli_h2("Fixed effects")
    print(var$fixed)
    cli::cli_h2("Random effects")
    print(var$random)
    cli::cli_h2("Likelihood ratio test")
    print(var$LRT)
    cli::cli_h2("Variance components and genetic parameters")
    print(var$ESTIMATES)
    cli::cli_h2("Principal component analysis of the G x E interaction matrix")
    print(var$PCA)
    if (blup == TRUE) {
      cli::cli_h2("BLUPs for genotypes")
      print(var$BLUPgen)
      cli::cli_h2("BLUPs for genotypes-vs-environments")
      print(var$BLUPgge)
    }
