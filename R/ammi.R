...
#' *j*th environment (\eqn{y_{ij}}) using the AMMI model, is given as follows:
#'\deqn{{y_{ij}} = \mu  + {\alpha_i} + {\tau_j} + \sum\limits_{k = 1}^p
#'{{\lambda_k}{a_{ik}}} {t_{jk}} + {\rho_{ij}} + {\varepsilon _{ij}}}
#'
#' where \eqn{\lambda_k} is the singular value for the *k*-th interaction
#' principal component axis (IPCA); \eqn{a_{ik}} is the *i*-th element of the
#' *k*-th eigenvector; \eqn{t_{jk}} is the *j*th element of the *k*th
#' eigenvector. A residual \eqn{\rho _{ij}} remains, if not all *p* IPCA are
#' used, where \eqn{p \le min(g - 1; e - 1)}.
#'
...
ammi <- function(.data,
  ...
  for (var in 1:nvar) {
    ...
    if(missing(block) || is.null(block)){
      ...
      augment <- model$model |>
        add_cols(hat = influence$hat,
                 ...
                 stdres = rstandard(model),
                 se.fit = predict(model, se.fit = TRUE)$se.fit) |>
        (\(x) add_cols(x, factors = concatenate(x, GEN, REP, BLOCK, pull = TRUE)))() |>
    ...
    if (minimo < 2) {
      cli::cli_abort("The analysis AMMI is not possible. Both genotypes and environments must have more than two levels.")
    }
  ...
  if (verbose == TRUE) {
    if (length(which(unlist(lapply(listres, function(x) {
      ...
      cli::cli_inform("{names(which(unlist(lapply(listres, function(x) { x[[\"probint\"]] })) > 0.05))}")
    } else {
      cli::cli_alert_success("All variables with significant (p < 0.05) genotype-vs-environment interaction")
    }
    cli::cli_alert_success("Done!")
  }
  invisible(structure(listres, class = c("ammi", "performs_ammi")))
}
...
performs_ammi <- function(.data,
                          ...
                          verbose = TRUE,
                          ...) {
  deprecated_warning("1.20.0", "performs_ammi()", "ammi()")
  ammi(
    .data = .data,
    env = {{env}},
    gen = {{gen}},
...
predict.performs_ammi <- function(object, naxis = 2, ...) {
  cal <- match.call()
  if (!inherits(object, "performs_ammi") && !inherits(object, "waas") && !inherits(object, "ammi")) {
    cli::cli_abort("The object must be of class 'ammi', 'performs_ammi', or 'waas'")
  }
  if (length(object) != length(naxis)) {
    cli::cli_abort("The argument 'naxix = ", cal[3], "' must length of ",
  ...
  for (var in 1:length(object)) {
    ...
    if (naxis[var] > minimo) {
      ...
      if (naxis[var] == 0) {
        cli::cli_abort("Invalid argument. The AMMI0 model is calculated automatically. Please, inform naxis > 0")
      } else {
...
print.performs_ammi <- function(x, export = FALSE, file.name = NULL, digits = 4, ...) {
  ...
  for (i in 1:length(x)) {
    ...
    cli::cli_h2("AMMI analysis table")
    print(var$ANOVA)
    cli::cli_h2("Scores for genotypes and environments")
    print(var$model)
    cli::cli_text("")
  }
  if (export == TRUE) {
...
#' @export
predict.ammi <- function(object, naxis = 2, ...) {
  predict.performs_ammi(object, naxis = naxis, ...)
}

#' @export
print.ammi <- function(x, export = FALSE, file.name = NULL, digits = 4, ...) {
  print.performs_ammi(x, export = export, file.name = file.name, digits = digits, ...)
}
